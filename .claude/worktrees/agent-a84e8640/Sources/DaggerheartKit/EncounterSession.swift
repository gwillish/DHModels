...
public final class EncounterSession: Identifiable, Hashable {
  ...
  public func reduceStress(_ amount: Int, to participant: some CombatParticipant) {
    let id = participant.id
    if modifying(
  ...
  public func markArmorSlot(_ slotID: UUID) {
  ...
  public func restoreArmorSlot(_ slotID: UUID) {
    guard let index = playerSlots.firstIndex(where: { $0.id == slotID }) else { return }
